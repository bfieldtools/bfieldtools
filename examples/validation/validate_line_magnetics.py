"""
The script checks the relation B = -mu0*grad(U) between the scalar potential and 
...
computed by threee different methods and prints out a relative error and a 
...
The script also compares the magnetic field of a rectangular loop to values
...
B_3 = 1/(2*d)*np.vstack([(A_yp[:, 2] - A_ym[:, 2]) - (A_zp[:, 1] - A_zm[:, 1]),
                         - (A_xp[:, 2] - A_xm[:, 2]) +
                         (A_zp[:, 0] - A_zm[:, 0]),
                         (A_xp[:, 1] - A_xm[:, 1]) - (A_yp[:, 0] - A_ym[:, 0])]).T

# Print the maximum relative deviation between the fields calculated by
# first two methods.
err = np.max(np.abs((B_2 - B)/(np.abs(B) + np.abs(B_2))))

if err < 5e-8:
    print('The scalar potential test is passed.')
else:
    print('The scalar potential test is failed. The relative error is expected '
          'to be around 3.6e-8.')
print(f'Relative error: {err}\n')


err = np.max(np.abs((B_2 - B_3)/(np.abs(B_3) + np.abs(B_2))))

if err < 2e-7:
    print('The vector potential test is passed.')
else:
    print('The vector potential test is failed. The relative error is expected '
          'to be around 1e-7.')
print(f'Relative error: {err}\n')
...
B_ref = [[5.0405815897640994e-8, 5.692103512792891e-9, 3.786395675311007e-8],
...
err = np.max(np.abs((B_lin - B_ref)/(np.abs(B_lin) + np.abs(B_ref))))

if err < 1e-9:
    print('The magnetic field test is passed.')
else:
    print('The magnetic field test is failed. The relative error is expected '
          'to be below 1e-9.')
print(f'Relative error: {err}')
