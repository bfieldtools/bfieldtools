...
def bfield_line_segments(vertices, points):
    ...
    for i in range(len(vertices) - 1):
        ...
        sinefactor = sinefactor - (d * a1).sum(axis=0) / np.sqrt(
            np.square(a1).sum(axis=0)
        )
...
if __name__ == "__main__":
    ...
    b2 = bfield_iterative(
        X, Y, Z, 0, 0, 0.2, 0.1, 1, 25
    )  # Calculates bfield iteratively

    # Error between two calculation methods.
    berr = (b2 - b1) / b1 * 100
    BE = berr.T[
        2
    ]  # By changing the index, errors in different components can be obtained
