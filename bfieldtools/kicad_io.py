...
def kicad_to_python(
    ...
    with open(filedir, "r") as infile:
        ...
        for line in infile:
            ...
            txt = line.split()
            if (
                len(txt) > 0 and txt[0] == "(segment" and txt[-2] == "(tstamp"
            ):  # Checks if there is something weird written on the segment lines
                txt = txt[0:-2]
            if not net:
                if (
                    len(txt) > 0
                    and txt[0] == "(segment"
                    and (txt[-3] == layers[0] or txt[-3] == layers[1])
                ):
                    if txt[-3] == layers[1]:
                        seg_start[k, norm_dir] = (
                            stand_off + stack * stack_spacing + 0.5 * stack_spacing
                        )  # for back layer of the stack the spacing is 1.5 timest the defined stack_spacing + standoff
                        seg_end[k, norm_dir] = (
                            stand_off + stack * stack_spacing + 0.5 * stack_spacing
                        )
                    else:
                        seg_start[k, norm_dir] = (
                            stand_off + stack * stack_spacing
                        )  # for back layer of the stack the spacing is stand_off + stack_spacing
                        seg_end[k, norm_dir] = stand_off + stack * stack_spacing

                    seg_start[k, plane_axes[0]] = (
                        float(txt[2]) / scale_factor - origin[0]
                    )
                    seg_end[k, plane_axes[0]] = float(txt[5]) / scale_factor - origin[0]
                    seg_start[k, plane_axes[1]] = (
                        float(txt[3]) / scale_factor - origin[1]
                    )
...
def segment_looper(seg_starts, seg_ends, start_ind, up_low):
    ...
    indices = np.arange(
        len(seg_starts)
    )  # initialize the list of indices of the segments
    match_order.append(start_ind)  # add the starting index to mathc_order
    indices = np.delete(
        indices, start_ind
    )  # delete the used index from indices to prevent using the index again

    matcher = seg_ends[
        start_ind
    ]  # The point to which we will next find a matching point
    matcher_layer = "front"  # indicates is the point on outer or inner layer of the stack (e.g. F.Cu or In1.Cu)

    l_spacings = np.sort(
        np.abs(np.unique(seg_starts[:, 2]))
    )  # Coordinate of the two layers towards the plane norm
    ...
    while len(indices):
        ...
        if up_low == 1:
            front_inds = indices[
                np.where(match_list[:, 2] == l_spacings[0])[0]
            ]  # indices for segments on front layer
            back_inds = indices[
                np.where(match_list[:, 2] == l_spacings[1])[0]
            ]  # indices for segments on back layer
        if up_low == 0:
        ...
        try:
            min_s_f = np.min(
                np.linalg.norm((seg_starts[front_inds, 0:2] - matcher[0:2]), axis=1)
            )  # Min to matcher distance from starting points on front layer
            min_e_f = np.min(
                np.linalg.norm((seg_ends[front_inds, 0:2] - matcher[0:2]), axis=1)
            )  # Min to matcher disntance from ending points on front layer

            min_front = np.min(
                np.array([min_s_f, min_e_f])
            )  # See which one has smaller distance to current matcher-point
            ...
            if min_s_f == min_front:
                match_ind_f = front_inds[
                    (
                        np.linalg.norm(
                            (seg_starts[front_inds, 0:2] - matcher[0:2]), axis=1
                        )
                    ).argmin()
                ]
            ...
            if min_e_f == min_front:
                match_ind_f = front_inds[
                    (
                        np.linalg.norm(
                            (seg_ends[front_inds, 0:2] - matcher[0:2]), axis=1
                        )
                    ).argmin()
                ]
        ...
        try:
            min_s_b = np.min(
                np.linalg.norm(seg_starts[back_inds, 0:2] - matcher[0:2], axis=1)
            )
            min_e_b = np.min(
                np.linalg.norm(seg_ends[back_inds, 0:2] - matcher[0:2], axis=1)
            )
            ...
            if min_s_b == min_back:
                match_ind_b = back_inds[
                    (
                        np.linalg.norm(
                            seg_starts[back_inds, 0:2] - matcher[0:2], axis=1
                        )
                    ).argmin()
                ]
